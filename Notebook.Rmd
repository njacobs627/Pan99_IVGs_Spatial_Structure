...
Fig.2 +
  ...
  geom_point(data = Inf.Unit.Sum,
             aes(x = Pp,
                 ...
                 color = factor(Rep)),
             size = 2) +
      geom_segment(data = Inf.Unit.Sum,
               aes(x = Pp, xend = Pp,
                   y = 0, yend = Full,
                   color = factor(Rep)),
               lty = 1) +
...
Fig.2 +
  ...
  xlab(bquote(bold(P[P]))) +
  scale_color_tableau(palette = "Tableau 20") +
  geom_segment(aes(x = 0, xend = 0.58,
                   y = 3.6, yend = 3.6),
               lty = 2) +
  # geom_segment(aes(x = 0.58, xend = 0.58,
  #                  y = 0, yend = 3.6),
  #              lty = 2) +
...
pp = c(seq(from = 0.1, to = 1, by = 0.1), 0.58)
moi = 10^(seq(from = -6, to = 3, by = 0.01))
n = 1:1000
...
df2 = df1 %>%
  mutate(p_V = dpois(x = V, lambda = MOI),
         ...
         pIncomplete_V = 1 - (1 - (1 - PP) ^ V) ^ 8,
         p8_NoComp = p_V * p8_V_NoComp,
         p8_Comp = p_V * p8_V_Comp,
         pIncomplete = p_V * pIncomplete_V) %>%
  group_by(PP,MOI) %>%
  summarise(p8_NoComp = sum(p8_NoComp),
            p8_Comp = sum(p8_Comp),
            pIncomplete = sum(pIncomplete) / (sum(pIncomplete) + sum(p8_Comp)))

df3 = df2 %>%
  ungroup() %>%
  mutate(p_Infected_Comp = pmin(p8_Comp * Cell.Num,1),
         p_Infected_NoComp = pmin(p8_NoComp * Cell.Num,1))
...
Fig.3 + 
  geom_line(data = df3 %>% filter(PP != 0.58),
            aes(x = MOI %>% log10,
                y = p_Infected_Comp * 100,
                group = PP,
                color = PP),
            lwd = 1) +
  scale_color_viridis(guide = FALSE) +
  scale_x_continuous(breaks = c(-6,-4,-2,0), limits = c(-6,1)) +
  labs(x = TeX("\\textbf{MOI (log_1_0 virions / cell)}"),
       y = TeX("\\textbf{Expected % populations infected}"),
       color = NULL)
...
Fig.3 +
  ...
  scale_y_continuous(limits = c(-6,2.1), breaks = c(-6,-4,-2,0,2)) +
